...
function Lobby({connect, playersUntilStart}) {
        ...
        <div className="App">
            {searching ?
                <div>
                    ...
                    {selectedGameModes.map(mode => <p key={mode}>{mode}</p>)}
                    <h1> Waiting for {playersUntilStart} more {playersUntilStart === 1 ? "player" : "players"}.</h1>
                </div>
                :
                <div>
                    {gameModes.map(mode =>
                        ...
                        }} style={{padding: '10px', cursor: 'default'}} key={mode}>
