...
function App() {
    ...
    const connect = ({playerCounts, gameModes}: SelectedGameModes) => {
        socket = io(backend);
        socket.on('connect', () => {
            console.log(`Connected to server. Your id is ${socket.id} and you're looking for games [${gameModes}] for [${playerCounts}] players.`);
    ...
    return (
        <div className="App">
            {game ? game : <Lobby connect={connect} playersConnected={playersConnected}/>}
        </div>
    );
}
