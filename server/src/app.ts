...
io.on('connection', (socket: SocketIO.Socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        if (waitingClientIds.includes(socket.id)) waitingClientIds.splice(waitingClientIds.indexOf(socket.id), 1);
    });

    // SMChat
    socket.on('updateText', (newText: string) => {
        rooms
            .find((room) => room.some((clientId: string) => clientId === socket.id))
            .forEach((socketId) => {
